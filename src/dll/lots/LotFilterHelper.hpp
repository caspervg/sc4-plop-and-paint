...
struct LotView {
    ...
    [[nodiscard]] std::string BuildingNodeID() const { return std::format("B{}", building->instanceId.value()); }
...
public:
    enum class WallToWallFilter : uint8_t { Any, WallToWallOnly, NonWallToWallOnly };
    ...
    std::unordered_set<uint32_t> selectedBuildingStyles;
    ...
    WallToWallFilter wallToWallFilter = WallToWallFilter::Any;
    bool activeStylesOnly = false;
    bool favoritesOnly = false; // If true, only show favorited lots

    [[nodiscard]] bool PassesFilters(const LotView& lot, const BuildingStyleContext& buildingStyleContext = {}) const;

    [[nodiscard]] std::vector<LotView> ApplyFiltersAndSort(const std::vector<LotView>& lots,
...
private:
    ...
    [[nodiscard]] bool PassesBuildingStyleFilter_(const LotView& lot, const BuildingStyleContext& context) const;
    [[nodiscard]] bool PassesWallToWallFilter_(const LotView& lot, const BuildingStyleContext& context) const;
    ...
    [[nodiscard]] bool PassesFavoritesOnlyFilter_(const LotView& lot,
